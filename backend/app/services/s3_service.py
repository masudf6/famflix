...
def delete_s3_object(key: str) -> None:
    ...
    S3 delete_object is idempotent: deleting a missing key is still considered
    successful by S3. Let real AWS/client errors bubble up to the route.
    """
    s3 = get_s3_client()
    s3.delete_object(Bucket=settings.AWS_S3_BUCKET, Key=key)


def upload_bytes_to_s3(key: str, body: bytes, content_type: str) -> None:
